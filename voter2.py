...
if __name__ == "__main__":
    ...
    ns = Pyro5.api.locate_ns()                          # Localiza o servidor de nomes Pyro
    voter1 = Voter()                                   # Cria a instância do broker votante 1
    uri = daemon.register(voter1)                       # Registra o objeto votante no daemon
    leader = Pyro5.api.Proxy(ns.lookup("Lider-Epoca1")) # Obtém o proxy do líder
    voter1.uri = uri                                    # Armazena o URI do votante para enviar o heartbeat
    print(leader.register_broker(uri, "votante"))       # Registra o votante 1 no líder com o estado "votante"
    voter1.request_state()                              # Requisita o estado atual do líder
    print("Votante 2 pronto.")                          # Exibe no terminal que o broker votante 1 está pronto
    
    # Inicia a thread para enviar heartbeats de forma contínua
    heartbeat_thread = threading.Thread(target=voter1.start_heartbeat)
